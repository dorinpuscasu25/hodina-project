...
export const MessagesPage = ({
  ...
  const [selectedBooking, setSelectedBooking] = useState<number | null>(selectedBookingId ?? null);
  ...
  const loadBookings = async () => {
    ...
    try {
      ...
      const availableConversations = response.data.filter((booking) => booking.chat_enabled);
      setBookings(availableConversations);
      setSelectedBooking((current) => current ?? availableConversations[0]?.id ?? null);
    } catch (loadError) {
  ...
  const loadMessages = async (bookingId: number) => {
    ...
    try {
      const response = await apiRequest<{ data: BookingMessage[] }>(`/client/bookings/${bookingId}/messages`, {
        token,
      });
  ...
  const selectedConversation = bookings.find((booking) => booking.id === selectedBooking) ?? null;
  const conversationMessages = selectedBooking ? messages[selectedBooking] ?? [] : [];
  ...
  const handleSendMessage = async (event: React.FormEvent) => {
    ...
    try {
      const response = await apiRequest<{ data: BookingMessage }>(`/client/bookings/${selectedBooking}/messages`, {
        token,
        method: 'POST',
        body: {
      ...
      <div className="min-h-screen bg-white pt-16">
        <div className="mx-auto max-w-4xl px-4 py-16 text-center sm:px-6 lg:px-8">
          <div className="rounded-3xl border border-gray-200 bg-white px-8 py-12 shadow-sm">
            ...
            <h2 className="text-2xl font-bold text-gray-900">Intră în cont pentru mesaje</h2>
            <button
      ...
      <div className="min-h-screen bg-white pt-16">
        <div className="mx-auto max-w-4xl px-4 py-16 sm:px-6 lg:px-8">
          <div className="rounded-3xl border border-gray-200 bg-white px-8 py-12 text-center shadow-sm">
            <h2 className="text-2xl font-bold text-gray-900">Confirmă emailul pentru a trimite mesaje</h2>
            <button
      ...
      <div className="flex flex-1 overflow-hidden">
        <div className="flex w-full flex-col border-r border-gray-200 bg-white md:w-96">
          <div className="border-b border-gray-200 p-4">
            <h1 className="mb-4 text-2xl font-bold text-gray-900">{t.messages.title}</h1>
          ...
          <div className="flex-1 overflow-y-auto">
            {isLoading ? (
              ...
              <div className="p-8 text-center text-gray-500">{t.messages.noConversations}</div>
            ) : (
              filteredBookings.map((booking) => (
                ...
                >
                  <div className="flex gap-3">
                    <img
                      src={booking.bookable?.cover_image ?? 'https://placehold.co/160x160?text=H'}
                    ...
                    <div className="min-w-0 flex-1">
                      <div className="mb-1 flex items-start justify-between gap-2">
                        <h3 className="truncate font-semibold text-gray-900">
                          {booking.guesthouse?.name ?? booking.bookable?.title ?? booking.booking_number}
                        </h3>
                        <span className="text-xs text-gray-500">{formatDateTime(booking.starts_at)}</span>
                      </div>
                      <p className="truncate text-sm text-gray-600">{booking.bookable?.title}</p>
                      <p className="truncate text-sm text-gray-500">{booking.booking_number}</p>
                    </div>
                  </div>
                </button>
              ))
        ...
        {selectedConversation ? (
          <div className="hidden flex-1 flex-col md:flex">
            <div className="flex items-center justify-between border-b border-gray-200 bg-white p-4">
              <div>
                <h2 className="font-semibold text-gray-900">{selectedConversation.guesthouse?.name ?? selectedConversation.bookable?.title}</h2>
                <p className="text-sm text-gray-600">{selectedConversation.bookable?.title}</p>
              </div>
              <button
                onClick={() =>
                  onNavigate('experience', {
                    ...
                    kind: selectedConversation.bookable_type === 'Accommodation' ? 'accommodation' : 'experience',
                  })
                }
            ...
            <div className="flex flex-1 overflow-hidden">
                        ...
                        <div className={`${isMine ? 'items-end' : ''} flex-1`}>
                          <div
                            className={`inline-block max-w-[70%] rounded-2xl px-4 py-2 ${
                              isMine ? 'bg-[#002626] text-white' : 'bg-gray-100 text-gray-900'
                            }`}
                          >
                            <p>{message.body}</p>
                          </div>
                          <p className="mt-1 px-2 text-xs text-gray-500">{formatDateTime(message.created_at)}</p>
                        </div>
                      </div>
                    );
                  })}
              ...
              <div className="w-80 overflow-y-auto border-l border-gray-200 bg-gray-50">
                <div className="p-4">
                  <h3 className="mb-4 font-semibold text-gray-900">{t.messages.reservation}</h3>
                  <div className="overflow-hidden rounded-xl bg-white shadow-sm">
                    <img
                      src={selectedConversation.bookable?.cover_image ?? 'https://placehold.co/800x500?text=Hodina'}
                      alt={selectedConversation.bookable?.title ?? selectedConversation.booking_number}
                      className="h-48 w-full object-cover"
                    />
                    <div className="space-y-3 p-4">
                      <h4 className="font-semibold text-gray-900">{selectedConversation.bookable?.title}</h4>
                      <div className="flex items-center gap-2 text-sm text-gray-600">
                        <Calendar className="h-4 w-4" />
                        <span>{formatDateTime(selectedConversation.starts_at)}</span>
                      </div>
                      <p className="text-sm text-gray-600">{selectedConversation.booking_number}</p>
                    </div>
                  </div>
                </div>
              </div>
          ...
          <div className="hidden flex-1 items-center justify-center bg-gray-50 md:flex">
            <div className="text-center">
              ...
              <p className="text-gray-500">{t.messages.selectConversationDesc}</p>
            </div>
          </div>
        )}
      </div>
